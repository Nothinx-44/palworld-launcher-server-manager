...
(function () {
  ...
  const T = {
    ...
    "Performances serveur": ["Server performance", "服务器性能", "Rendimiento del servidor", "서버 성능", "Server-Leistung", "サーバーパフォーマンス", "Desempenho do servidor"],
    "RAM (Mo)": ["RAM (MB)", "内存 (MB)", "RAM (MB)", "RAM (MB)", "RAM (MB)", "RAM (MB)", "RAM (MB)"],
    "Mo": ["MB", "MB", "MB", "MB", "MB", "MB", "MB"],
    "Diagnostic": ["Diagnostics", "诊断", "Diagnóstico", "진단", "Diagnose", "診断", "Diagnóstico"],
    "État des sous-systèmes du dashboard et du serveur, pour repérer rapidement ce qui bloque.": ["State of the dashboard and server subsystems, to quickly spot what's wrong.", "仪表盘与服务器各子系统的状态，帮助快速定位问题。", "Estado de los subsistemas del panel y del servidor, para detectar rápidamente qué falla.", "대시보드와 서버 하위 시스템의 상태 — 무엇이 문제인지 빠르게 파악하기 위한 정보입니다.", "Zustand der Dashboard- und Server-Subsysteme, um Probleme schnell zu erkennen.", "ダッシュボードとサーバーの各サブシステムの状態 — 問題を素早く特定するためのものです。", "Estado dos subsistemas do painel e do servidor, para identificar rapidamente o que está errado."],
    "Rafraîchir": ["Refresh", "刷新", "Actualizar", "새로고침", "Aktualisieren", "更新", "Atualizar"],
    "API du serveur joignable": ["Server API reachable", "服务器 API 可访问", "API del servidor accesible", "서버 API 연결 가능", "Server-API erreichbar", "サーバーAPIに接続可能", "API do servidor acessível"],
    "PalDefender configuré": ["PalDefender configured", "PalDefender 已配置", "PalDefender configurado", "PalDefender 구성됨", "PalDefender konfiguriert", "PalDefender 設定済み", "PalDefender configurado"],
    "Discord configuré": ["Discord configured", "Discord 已配置", "Discord configurado", "Discord 구성됨", "Discord konfiguriert", "Discord 設定済み", "Discord configurado"],
    "Espace disque suffisant": ["Enough disk space", "磁盘空间充足", "Espacio en disco suficiente", "디스크 공간 충분", "Ausreichend Speicherplatz", "ディスク空き容量あり", "Espaço em disco suficiente"],
    "Port de jeu": ["Game port", "游戏端口", "Puerto de juego", "게임 포트", "Spiel-Port", "ゲームポート", "Porta do jogo"],
    "Annonces récurrentes": ["Recurring announcements", "循环公告", "Anuncios recurrentes", "반복 공지", "Wiederkehrende Ankündigungen", "定期アナウンス", "Anúncios recorrentes"],
    "Activer les annonces récurrentes": ["Enable recurring announcements", "启用循环公告", "Activar anuncios recurrentes", "반복 공지 활성화", "Wiederkehrende Ankündigungen aktivieren", "定期アナウンスを有効にする", "Ativar anúncios recorrentes"],
    "Intervalle": ["Interval", "间隔", "Intervalo", "간격", "Intervall", "間隔", "Intervalo"],
    "minutes entre chaque message": ["minutes between each message", "每条消息之间的分钟数", "minutos entre cada mensaje", "각 메시지 사이의 분", "Minuten zwischen den Nachrichten", "各メッセージの間隔（分）", "minutos entre cada mensagem"],
    "Messages (diffusés en boucle)": ["Messages (broadcast in a loop)", "消息（循环播放）", "Mensajes (difundidos en bucle)", "메시지 (반복 전송)", "Nachrichten (in Schleife gesendet)", "メッセージ（ループで配信）", "Mensagens (transmitidas em loop)"],
    "Nouveau message…": ["New message…", "新消息…", "Nuevo mensaje…", "새 메시지…", "Neue Nachricht…", "新しいメッセージ…", "Nova mensagem…"],
    "Aucun message — ajoutes-en un.": ["No messages — add one.", "暂无消息——请添加一条。", "Sin mensajes: añade uno.", "메시지가 없습니다 — 하나 추가하세요.", "Keine Nachrichten — füge eine hinzu.", "メッセージがありません — 追加してください。", "Sem mensagens — adicione uma."],
    "Presets de difficulté :": ["Difficulty presets:", "难度预设：", "Ajustes de dificultad:", "난이도 프리셋:", "Schwierigkeits-Presets:", "難易度プリセット：", "Predefinições de dificuldade:"],
    "⏸️ Annonces récurrentes désactivées.": ["⏸️ Recurring announcements disabled.", "⏸️ 循环公告已禁用。", "⏸️ Anuncios recurrentes desactivados.", "⏸️ 반복 공지가 비활성화되었습니다.", "⏸️ Wiederkehrende Ankündigungen deaktiviert.", "⏸️ 定期アナウンスを無効にしました。", "⏸️ Anúncios recorrentes desativados."],
    "Annonces récurrentes activées": ["Recurring announcements enabled", "循环公告已启用", "Anuncios recurrentes activados", "반복 공지 활성화됨", "Wiederkehrende Ankündigungen aktiviert", "定期アナウンスを有効にしました", "Anúncios recorrentes ativados"],
    "Note enregistrée": ["Note saved", "备注已保存", "Nota guardada", "메모 저장됨", "Notiz gespeichert", "メモを保存しました", "Nota salva"],
    "Note sur": ["Note on", "关于", "Nota sobre", "메모 대상:", "Notiz zu", "メモ対象：", "Nota sobre"],
    "(vide pour effacer)": ["(empty to clear)", "（留空以清除）", "(vacío para borrar)", "(비우면 삭제)", "(leer zum Löschen)", "（空欄で削除）", "(vazio para apagar)"],
    "Échec de l'enregistrement": ["Failed to save", "保存失败", "Error al guardar", "저장 실패", "Speichern fehlgeschlagen", "保存に失敗しました", "Falha ao salvar"],
    "Preset appliqué": ["Preset applied", "已应用预设", "Ajuste aplicado", "프리셋 적용됨", "Preset angewendet", "プリセットを適用しました", "Predefinição aplicada"],
    "vérifie puis Enregistrer": ["review then Save", "请检查后保存", "revisa y luego guarda", "확인 후 저장하세요", "prüfen, dann Speichern", "確認して保存してください", "revise e salve"],
    "Aucun réglage correspondant": ["No matching setting", "没有匹配的设置", "Ningún ajuste correspondiente", "일치하는 설정 없음", "Keine passende Einstellung", "該当する設定なし", "Nenhuma configuração correspondente"],
    "Casual": ["Casual", "休闲", "Casual", "캐주얼", "Locker", "カジュアル", "Casual"],
    "Normal": ["Normal", "普通", "Normal", "보통", "Normal", "ノーマル", "Normal"],
    "Hardcore": ["Hardcore", "硬核", "Difícil", "하드코어", "Hardcore", "ハードコア", "Difícil"],
  };
  ...
  const PATTERNS = [
    ...
    [/ a diffusé une annonce récurrente(?=$| — )/, " broadcast a recurring announcement", " 发送了一条循环公告", " difundió un anuncio recurrente", " 님이 반복 공지를 전송했습니다", " hat eine wiederkehrende Ankündigung gesendet", " が定期アナウンスを配信しました", " transmitiu um anúncio recorrente"],
    [/ a modifié les annonces récurrentes(?=$| — )/, " changed the recurring announcements", " 修改了循环公告", " cambió los anuncios recurrentes", " 님이 반복 공지를 변경했습니다", " hat die wiederkehrenden Ankündigungen geändert", " が定期アナウンスを変更しました", " alterou os anúncios recorrentes"],
    [/ a modifié une note de joueur(?=$| — )/, " edited a player note", " 修改了一条玩家备注", " editó una nota de jugador", " 님이 플레이어 메모를 수정했습니다", " hat eine Spielernotiz bearbeitet", " がプレイヤーのメモを編集しました", " editou uma nota de jogador"],
  ];
