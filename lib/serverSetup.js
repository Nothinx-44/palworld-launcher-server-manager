...
async function runInstall(config, onLog) {
  ...
  // Le dossier WindowsServer n'est pas toujours créé par le 1er lancement au moment où l'on
  // copie DefaultPalWorldSettings.ini → PalWorldSettings.ini : sans ce mkdir, copyFileSync
  // échouait en ENOENT sur la destination (cas réel remonté par un utilisateur). Idem si le
  // fichier source a disparu entre-temps : on donne un message clair plutôt qu'un ENOENT brut.
  fs.mkdirSync(configDir, { recursive: true });
  if (!fs.existsSync(defaultSettingsFile)) {
    throw new Error(`DefaultPalWorldSettings.ini introuvable dans "${serverDir}" — le premier lancement du serveur ne l'a pas généré. Réessaie l'installation.`);
  }
